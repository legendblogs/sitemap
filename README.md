# Generate advance sitemaps with Legendblogs
...
```php
use Legendblogs\Sitemap\SitemapGenerator;
...
```php
use Carbon\Carbon;
use Legendblogs\Sitemap\Sitemap;
...
The generator has [the ability to execute JavaScript](https://github.com/legendblogs/sitemap#executing-javascript) on each page so links injected into the dom by JavaScript will be crawled as well.
...
``` bash
composer require legendblogs/sitemap
```

The package will automatically register itself.

If you want to update your sitemap automatically and frequently you need to perform [some extra steps](https://github.com/legendblogs/sitemap#generating-the-sitemap-frequently).
...
```bash
php artisan vendor:publish --provider="Legendblogs\Sitemap\SitemapServiceProvider"
```
...
You can create a custom crawl profile by implementing the `Legendblogs\Crawler\CrawlProfile` interface and by customizing the `shouldCrawl()` method for full control over what url/domain/sub-domain should be crawled:

```php
use Legendblogs\Crawler\CrawlProfile;
...
The crawler itself can be [configured](https://github.com/legendblogs/crawler#usage) to do a few different things.
...
Under the hood, [headless Chrome](https://github.com/legendblogs/browsershot) is used to execute JavaScript. Here are some pointers on [how to install it on your system](https://github.com/legendblogs/browsershot#requirements).
...
```php
use Legendblogs\Sitemap\SitemapIndex;
...
You can pass a `Legendblogs\Sitemap\Tags\Sitemap` object to manually set the `lastModificationDate` property.
